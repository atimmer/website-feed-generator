...
function parseArgs(argv: string[]): string[] {
  ...
  for (const token of argv) {
    ...
    if (token === "--timeout" || token.startsWith("--timeout=")) {
      console.error(
        "[runner] --timeout is no longer supported; rely on the automatic timeouts.",
      );
      process.exit(1);
    }
...
function determineEffectiveTimeoutMs(commandArgs: string[]): number {
  ...
  if (
    shouldExtendTimeout(commandArgs) &&
    !isSingleTestInvocation(commandArgs)
  ) {
    return EXTENDED_TIMEOUT_MS;
  }
...
function shouldExtendTimeout(commandArgs: string[]): boolean {
  ...
  return rest.some(
    (token) =>
      shouldExtendForScript(token) || TEST_BINARIES.has(token.toLowerCase()),
  );
}
...
function shouldExtendViaPnpm(rest: string[]): boolean {
  ...
  if (subcommand === "exec") {
    ...
    if (
      execTarget &&
      (shouldExtendForScript(execTarget) ||
        TEST_BINARIES.has(execTarget.toLowerCase()))
    ) {
      return true;
    }
    return rest
      ...
      .some(
        (token) =>
          shouldExtendForScript(token) ||
          TEST_BINARIES.has(token.toLowerCase()),
      );
  }
...
function shouldUseLintTimeoutViaPnpm(rest: string[]): boolean {
  ...
  if (subcommand === "exec") {
    ...
    return rest
      .slice(1)
      .some((token) => LINT_BINARIES.has(token.toLowerCase()));
  }
...
function matchesScriptKeyword(
  script: string,
  keywords: readonly string[],
): boolean {
  const lowered = script.toLowerCase();
  return keywords.some(
    (keyword) => lowered === keyword || lowered.startsWith(`${keyword}:`),
  );
}
...
function shouldUseLongTimeout(commandArgs: string[]): boolean {
  ...
  const matches = (token: string): boolean =>
    matchesScriptKeyword(token, LONG_SCRIPT_KEYWORDS);
...
async function runCommand(context: RunnerExecutionContext): Promise<void> {
  const { command, args, env } = buildExecutionParams(
    context.commandArgs,
  ...
  try {
    ...
    console.error(
      formatCompletionSummary({ exitCode, elapsedMs, commandLabel }),
    );
...
async function runCommandWithoutTimeout(
...
function injectWorkspaceBinDirs(
  ...
  if (ENABLE_DEBUG_LOGS) {
    console.error(
      `[runner] Prepending workspace PATH entries: ${additions.join(", ")}`,
    );
  }
...
function exitCodeFromSignal(signal: NodeJS.Signals | null): number {
  ...
  const code = (osConstants.signals as Record<string, number | undefined>)[
    signal
  ];
...
async function resolveCommandInterception(
  ...
  const gitContext = analyzeGitExecution(
    context.commandArgs,
    context.workspaceDir,
  );
...
function enforceGitPolicies(gitContext: GitExecutionContext) {
  ...
  const hasConsentOverride =
    process.env.RUNNER_THE_USER_GAVE_ME_CONSENT === "1";
...
async function maybeHandleFindInvocation(
  context: RunnerExecutionContext,
): Promise<boolean> {
  ...
  const findPlan = await buildFindDeletePlan(
    findInvocation.argv,
    context.workspaceDir,
  );
  if (!findPlan) {
  ...
  const moveResult = await movePathsToTrash(
    findPlan.paths,
    context.workspaceDir,
    { allowMissing: false },
  );
...
async function maybeHandleRmInvocation(
  context: RunnerExecutionContext,
): Promise<boolean> {
...
async function maybeHandleGitRm(
  ...
  const gitRmPlan = parseGitRmArguments(
    gitContext.invocation.argv,
    gitContext.command,
  );
...
async function maybeHandleSleepInvocation(
  context: RunnerExecutionContext,
): Promise<boolean> {
  ...
  for (let offset = 0; offset < rest.length; offset += 1) {
    ...
    adjustedArgs[commandIndex + 1 + offset] =
      formatSleepArgument(MAX_SLEEP_SECONDS);
  }
...
async function maybeHandleTmuxInvocation(
  context: RunnerExecutionContext,
): Promise<boolean> {
  const tokens = stripWrappersAndAssignments(context.commandArgs);
  if (tokens.length === 0) {
...
async function movePathsToTrash(
  ...
  if (trashCliCommand) {
    try {
      ...
      if (ENABLE_DEBUG_LOGS && stderrText.trim().length > 0) {
        console.error(
          `[runner] trash-cli error (${trashCliCommand}): ${stderrText.trim()}`,
        );
      }
    } catch (error) {
      if (ENABLE_DEBUG_LOGS) {
        console.error(
          `[runner] trash-cli invocation failed: ${formatTrashError(error)}`,
        );
      }
    }
  }
  ...
  for (const item of existing) {
    try {
      ...
      errors.push(
        `Failed to move ${item.raw} to Trash: ${formatTrashError(error)}`,
      );
    }
  }
...
function buildTrashTarget(trashDir: string, absolutePath: string): string {
  ...
  while (existsSync(candidate)) {
    candidate = join(
      trashDir,
      `${baseName}-${timestamp}${attempt > 0 ? `-${attempt}` : ""}`,
    );
    attempt += 1;
  }
  return candidate;
...
function formatCompletionSummary(options: {
  ...
  const durationText =
    typeof elapsedMs === "number" ? formatDuration(elapsedMs) : null;
